    ...
    <div className="space-y-6">
        ...
        <table className="army-table">
          ...
          <tbody>
            {isLoading ? (
              ...
              (entries as AuditLog[]).map((entry) => {
                const meta = ACTION_LABELS[entry.action] ?? { label: entry.action, badge: "badge-info" };
                ...
                return (
                  <tr key={entry.id}>
                    ...
                    <td>
                      <span className="font-medium text-[color:var(--text-primary)]">@{entry.admin_username}</span>
                    </td>
                    <td>
                      <span className={`badge ${meta.badge}`}>{meta.label}</span>
                    </td>
                    <td className="text-[color:var(--text-muted)]">{entry.target_label}</td>
                  </tr>
                );
              })
            )}
      ...
      {!session && (
        <div className="glass-card p-4 flex items-center justify-between gap-4">
          ...
          <a href="/login" className="btn-accent text-sm py-2 px-4 shrink-0">Ingresar</a>
        </div>
      )}
    </div>
  );
